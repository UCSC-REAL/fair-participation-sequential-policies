...
def main(problems: dict[str, dict]):
    # TODO clean dir version
    # Create needed directories if they don't exist
    for folder in ("losses", "data", "mp4", "npy", "pdf"):
        os.makedirs(folder, exist_ok=True)

    for problem, kwargs in problems.items():
        run_problem(problem, method="RRM", save_init=False, **kwargs)
        # run_problem(problem, method="RRM_grad", save_init=False, jit=True, **kwargs)
        run_problem(problem, method="LPU", save_init=False, **kwargs)
        # # run_problem(problem, method="LPU_grad", save_init=False, jit=True, **kwargs)
        run_problem(problem, method="Fair", save_init=False, **kwargs)
        # # run_problem(problem, method="Fair_grad", save_init=False, jit=True, **kwargs)
        run_problem(
            problem,
            method=None,
            save_init=True,
